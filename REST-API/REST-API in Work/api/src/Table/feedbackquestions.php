...
function getFeedbackquestions(){
  ...
  $sql = "SELECT * FROM feedbackquestions";
...
//Löscht alle items aus der Tabelle Feedbackquestions mit der QuestionID
function deleteFeedbackquestionsWithQuestionID($QuestionID){
  define('DOC_ROOT_PATH', $_SERVER['DOCUMENT_ROOT'].'/');
  include_once DOC_ROOT_PATH . "/othermethods.php";
  ...
  $sql = "DELETE FROM feedbackquestions WHERE QuestionID =$QuestionID";
  if ($conn->query($sql) == TRUE) {
    deliver_response(200, 0, "Data deleted successfully");
  }
  else {
