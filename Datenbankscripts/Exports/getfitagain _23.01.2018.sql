...
--
-- Host: 127.0.0.1
-- Erstellungszeit: 23. Jan 2018 um 18:15
...
CREATE TABLE `answer` (
  ...
  `answerstring` varchar(250) CHARACTER SET utf8 NOT NULL
...
INSERT INTO `answer` (`AnswerID`, `CustomerID`, `QuestionID`, `answerint`, `answerbool`, `answerstring`) VALUES
(1, 1, 1, 3, 0, 'IGNORESTRING'),
(2, 1, 2, 4, 0, 'IGNORESTRING'),
(3, 1, 3, 0, 1, 'IGNORESTRING'),
(4, 1, 4, 0, 0, 'lololololololololololololololol'),
(5, 1, 1, 3, 0, 'IGNORESTRING'),
(6, 1, 2, 5, 0, 'IGNORESTRING'),
(7, 1, 3, 0, 1, 'IGNORESTRING'),
(8, 1, 4, 0, 0, 'Sonstige Anmerkungen sind sehr beliebt');
...
(1, 'Wie gefällt es Ihnen bei uns?'),
(2, 'Anderes Feedback'),
(3, 'Neues Feedback');
...
CREATE TABLE `question` (
  `QuestionID` int(11) NOT NULL,
  `questiontext` varchar(250) CHARACTER SET latin1 NOT NULL,
  `kindofquestion` tinyint(4) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(1, 'Wieoft üben Sie in der Woche?', 1),
(2, 'Wieoft üben Sie im Monat?', 1),
(3, 'Trinken Sie Alkohol?', 2),
(4, 'Sonstige Anmerkungen', 3);
...
ALTER TABLE `answer`
  ...
  ADD CONSTRAINT `answer_ibfk_2` FOREIGN KEY (`CustomerID`) REFERENCES `customer` (`CustomerID`);
...
ALTER TABLE `feedbackdonebyuser`
  ...
  ADD CONSTRAINT `feedbackdonebyuser_ibfk_3` FOREIGN KEY (`CustomerID`) REFERENCES `customer` (`CustomerID`);
